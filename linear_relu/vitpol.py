...
class Attention(nn.Module):
    def __init__(self, dim, heads, dropout=0.1):
        ...
        # learnable exponent for each head
    ...
    def phi(self, x):
        # p_h = 1 + softplus(...)
        p = 1.0 + F.softplus(self.p_raw).view(1, self.heads, 1, 1)

        return F.relu(x).pow(p) + 1e-6
    ...
    def forward(self, x):
        ...
        z = 1.0 / (q @ k_sum.unsqueeze(-1) + 1e-6)

        out = (q @ kv) * z

        # сохраняем энергию нормировки для анализа
        self.last_energy = z.detach()
