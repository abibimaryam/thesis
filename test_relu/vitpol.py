...
class Attention(nn.Module):
    def __init__(self, dim, heads, dropout=0.1):
        ...
        # one scalar projection Pi_h per head
        self.slice_q = nn.Parameter(torch.randn(heads, self.head_dim))
        self.slice_k = nn.Parameter(torch.randn(heads, self.head_dim))

        # learnable exponent per head
        self.p_raw = nn.Parameter(torch.zeros(heads))
    ...
    def forward(self, x):
        ...
        qkv = self.to_qkv(x).chunk(3, dim=-1)
        q, k, v = map(
            lambda t: t.reshape(B, N, self.heads, self.head_dim).transpose(1, 2),
            qkv
        )   # [B,H,N,D]

        # -----------------------------
        # SLICED 1D projections
        # -----------------------------
        q_proj = (q * self.slice_q.view(1, self.heads, 1, self.head_dim)).sum(dim=-1)  # [B,H,N]
        k_proj = (k * self.slice_k.view(1, self.heads, 1, self.head_dim)).sum(dim=-1)  # [B,H,N]

        # pairwise projected differences
        diff = q_proj.unsqueeze(-1) - k_proj.unsqueeze(-2)   # [B,H,N,N]

        # learnable exponent
        p = 1 + F.softplus(self.p_raw).view(1, self.heads, 1, 1)

        # numerator: power-ReLU interactions
        num = F.relu(diff) ** p

        # denominator: absolute interaction energy (paper normalization)
        den = diff.abs().sum(dim=-1, keepdim=True) + 1e-6

        attn = num / den

        # -----------------------------
        # centered values (paper trick)
        # -----------------------------
        v_centered = v - v.mean(dim=2, keepdim=True)

        self.last_attn = attn.detach()
        self.last_scores = diff.detach()

        attn = self.attn_dropout(attn)

        out = (attn @ v_centered).transpose(1, 2).reshape(B, N, C)
