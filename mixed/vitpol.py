...
class Attention(nn.Module):
    def __init__(self, dim, heads, dropout=0.1, use_softmax=True):
        super().__init__()
        self.heads = heads
        ...
        # Для ReLU+P варианта
        self.p_raw = nn.Parameter(torch.tensor(0.0))  # один параметр на все головы
    ...
    def forward(self, x):
        ...
        if self.use_softmax:
            ...
            # Улучшенный ReLU + степень
            p = 1 + torch.sigmoid(self.p_raw) * 3  # p ∈ [1, 4]
            
            # Clip для численной стабильности
            attn_clipped = torch.clamp(attn, min=0.0, max=10.0)
            attn = torch.pow(attn_clipped, p)
            
            # L1 нормализация вместо суммы (даёт разреженность)
            attn = attn / (torch.norm(attn, p=1, dim=-1, keepdim=True) + 1e-8)
            
            # Логируем разреженность
            self.last_sparsity = (attn < 1e-6).float().mean().item()
...
class MixtureAttention(nn.Module):
    """
    Динамическое переключение между Softmax и ReLU+P attention
    """
    ...
    def forward(self, x):
        ...
        # Softmax ветка
        soft_attn = F.softmax(attn_logits, dim=-1)
        
        # ReLU+P ветка
        p = 1 + torch.sigmoid(self.p_raw) * 3
        sparse_attn = torch.pow(torch.clamp(attn_logits, min=0.0), p)
        sparse_attn = sparse_attn / (sparse_attn.sum(dim=-1, keepdim=True) + 1e-8)
        
        # Динамическая смесь
        alpha = torch.sigmoid(self.mix_weight)
        attn = alpha * soft_attn + (1 - alpha) * sparse_attn
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, heads, mlp_dim, dropout=0.1):
        ...
        self.attn = MixtureAttention(dim, heads, dropout)
